  ...
  router.beforeEach(async (to) => {
    ...
    if (!token || isTokenExpired(token)) {
      ...
      if (isFromLogout) {
        ...
        if (isFromUrl || isExpired) {
          ...
          // NOTE: don't show the session-expired toast here; we'll only show
          // a warning when an actual API call fails with 401 (handled below).
        }
      }
    ...
    if (!userStore.userInfo) {
      try {
        ...
        // Show session expired message only when API returned 401
        try {
